...
Private local research project implementing Google CURIE scientific long-context tasks with Recursive Language Models. The default policy model is `Qwen/Qwen3-8B`. The earlier `Qwen/Qwen3.5-7B-Instruct` was removed because it is not a valid Hugging Face repo (no Qwen3.5 family exists on HF — only Qwen2.5 and Qwen3); override the default at the prime-rl CLI with `--model.name <hf_repo>` if needed. The environment inherits the official `verifiers.envs.experimental.rlm_env.RLMEnv` for recursion and long-context execution, with the sandbox backend overridden to a local Docker daemon and the sub-LLM interception URL pinned to the local pod — there is no opt-out path back to Prime hosted services from this repo.

## Local runtime (the only supported runtime)

Local single-pod training is the only supported configuration. `CurieRLMEnv` runs sandboxes on the **local Docker daemon** and resolves a sandbox→env-worker callback URL locally — both the `prime_sandboxes` hosted backend and the `prime_tunnel` hosted tunnel are unconditionally bypassed. There is no opt-out path back to Prime hosted services. `PRIME_API_KEY` is not used by any local-training path; do not set it expecting different behavior.
...
|---|---|---|
| `CURIE_SANDBOX_BACKEND` | `local_docker` | Only `local_docker` is accepted; any other value raises at startup |
...
Exits 0 when local mode is healthy (Docker reachable, backend = local_docker, interception local) and 1 otherwise.
