...
- Free-form reference preprocessing (`CurieRubric._freeform_reference`): verbatim port of `preprocess_ground_truth` from `data/curie/colabs/curie_run_eval.ipynb` cell 28, else-branch (which is the path all five free-form tasks DFT-C/HFE/HFD/QECC_65/GEO take). Curie's scorer parses the json-encoded GT, pops three identifier fields (`record_id`, `arxiv_id`, `paper_id` — top-level for dict GTs, per-item for list GTs), then passes `str(json5.dumps(json_gt))` of the cleaned object to ROUGE/BERT. NOT a deviation — this is how Curie itself scores. An earlier commit on this branch tried per-task field whitelists (HFE Hamiltonian+Other_info, HFD task+Note per step, DFT-C `code`, etc.) on the misreading that Curie scored extracted prose; cell 28 verifies it does not. The per-task whitelists were removed. Applied in `_freeform_geometric_reward` (headline) and `_aux_rouge_lsum`/`_aux_bert_f1` (observability) so headline and aux numbers stay consistent, and only for tasks in `_FREEFORM_TASKS` since LLMSim/IoU/IDr have their own structured parsers.
