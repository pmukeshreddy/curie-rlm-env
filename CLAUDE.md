...
Stage 5 RL training is split into 3 phases (curriculum) per Stage 5a judge-bottleneck analysis:
- Phase 1 — free-form (programmatic only, fast): DFT-C, HFE, HFD, QECC_65, GEO
- Phase 2 — retrieval (LLMSim in loop, judge cache active, rollouts/ex = 4): DFT-S, DFT-P, MPVE
- Phase 3 — geometric/structural (programmatic, fast): BIOGR, PDB
Each phase resumes from the prior phase's checkpoint.

Stage 5.5 RFT contingency:
- Triggered only if Stage 5 phase1 stalls (no reward improvement over baseline + 0.05 in 500 steps, OR variance > 2x reward mean).
...
- After RFT, Stage 5 phase1 resumes from the RFT'd checkpoint.

Stage 6 skipped. Original placeholder ("mixing + online refinement") had no concrete deliverable. prime-rl handles async off-policy correction internally. Final pipeline: 0 → 1 → 2 → 3 → 3.5 → [4 skip] → 5b → 5.5 (contingency) → [6 skip] → 6.5 (verification) → 7 → 8 (optional).
