...
- Free-form reference extraction (`CurieRubric._freeform_reference`): the dataset's `answer` field for free-form tasks is `json.dumps(entry["ground_truth"])` (datasets.py:148). The GT is a structured object whose JSON serialization mixes content with structural keys (`{`, `}`, `"key":`) and identifier metadata (`record_id`, `arxiv_id`, `paper_link`, `code_id`); passing the json-encoded answer directly to ROUGE/BERT injects both as scoring noise. The rubric now extracts content with an explicit per-task field whitelist before scoring (NOT a generic "concatenate every string" collector — that approach treated `Hamiltonian` (LaTeX) the same as `Score` (numeric) for HFE and concatenated `Equation` LaTeX with step prose for HFD, both of which still injected noise). Per-task choices:
  - DFT-C: read `info["dft_field"]` (== `"code"` per `TASK_MAP[DFT-C]`) and extract that field. Source-of-truth, not an assumption.
  - HFE: extract `Hamiltonian` + `Other_info`. ASSUMPTION pending verification against `data/curie/colabs/curie_run_eval.ipynb` cell 30 — the alternative is `Hamiltonian` alone.
  - HFD: per step, extract `task` + `Note` (excludes `Step` integer + `Equation` LaTeX). ASSUMPTION pending cell 30.
  - QECC_65: per record, extract `name` + `physical` + `logical` + `introduced` (excludes `code_id` and any other identifier). ASSUMPTION pending cell 30.
  - GEO: extract `notes` + `paper_title` (excludes `paper_link`, `record_id`, `datasets` list-of-identifiers). ASSUMPTION pending cell 30.

  Two-layer fallback when per-task extraction returns nothing: (1) generic `_collect_content_strings` recursive collector skipping identifier-like keys, (2) original `answer` string. Both layers preserve behavior on unknown shapes instead of zeroing the reward. Applied identically in `_freeform_geometric_reward` (headline) and `_aux_rouge_lsum`/`_aux_bert_f1` (observability) so headline and aux numbers stay consistent. Discovered after the BERTScore rescale revert when the Stage 3b harness showed BERT raw clustering at 0.72-0.80 across all 16 rollouts; investigation traced part of the compression to the json-encoded reference (the rest is the structural prose-vs-code/LaTeX/dict mismatch, which only changes as the policy learns to produce GT-shaped outputs).
